...
export default function SubscriptionPage() {
  ...
  const handleSelectPackage = async (pkg: any) => {
    // منع اختيار الباقة بدون تسجيل الدخول
    let effectiveUserId = user?.id as string | undefined
    if (!effectiveUserId && typeof localStorage !== 'undefined') {
      try {
        const storedUserStr = localStorage.getItem('user')
    ...
    if (!effectiveUserId) {
      toast.error('يرجى تسجيل الدخول لاختيار الباقة')
      router.push('/login')
      return
    }
  ...
  const handlePayment = async () => {
    ...
    if (!receiptImage) {
      toast.error('يجب إرفاق صورة إيصال الدفع')
      return
    }
    
    if (!selectedPackage) {
      toast.error('بيانات الباقة غير متوفرة')
      return
    }
    ...
    if (!effectiveUserId) {
      toast.error('يرجى تسجيل الدخول مرة أخرى قبل إرسال الإيصال')
      return
    }
    ...
    try {
      ...
      // بيانات الباقة والمستخدم - تأكد من وجودها وصحتها
      formData.append('packageId', selectedPackage.id)
      formData.append('packageName', selectedPackage.name)
      
      // إرسال معرف المستخدم بشكل مباشر مع النموذج
      console.log('إرسال معرف المستخدم:', effectiveUserId);
      ...
      formData.append('paymentMethod', 'vodafone')
      
      // تأكد من وجود صورة الإيصال
      if (receiptImage) {
      ...
      {showPaymentModal && selectedPackage && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
          <motion.div
            ...
            className="bg-gradient-to-br from-black via-gray-900 to-black p-6 rounded-2xl w-full max-w-md border-2 border-gold shadow-xl shadow-gold/30 backdrop-blur"
          >
            <div className="text-center mb-6">
              <h2 className="text-3xl font-bold bg-gradient-to-r from-gold via-yellow-400 to-gold bg-clip-text text-transparent mb-1">تفاصيل الدفع</h2>
              <p className="text-white/70">الباقة: {selectedPackage.name}</p>
            </div>

            <div className="space-y-6">
              {/* معلومات الدفع */}
              <div className="p-4 bg-gradient-to-br from-black/40 to-black/70 rounded-xl border-2 border-gold/40 shadow-lg shadow-gold/20">
                <div className="flex justify-between items-center border-b border-white/10 pb-3 mb-3">
                  <span className="text-white/70">المبلغ المطلوب:</span>
                  <span className="text-xl font-bold bg-gradient-to-r from-gold to-yellow-400 bg-clip-text text-transparent">
              ...
              <div className="mt-4 space-y-2">
                ...
                <div className="flex items-center justify-center">
                  <button
                    type="button"
                    className="w-full py-3 px-4 rounded-lg font-bold flex items-center justify-center gap-2 bg-gradient-to-r from-gold to-yellow-500 text-black hover:shadow-lg hover:shadow-gold/40 border-2 border-gold/30"
                  >
                    <Phone size={20} />
                    فودافون كاش
                  </button>
              ...
              <div className="mt-6">
                <p className="text-sm font-medium text-white/70 mb-2">
                  صورة إيصال الدفع 
                  <span className="text-gold"> (مطلوب)</span>
                </p>
                <div className="border-2 border-dashed border-gold/30 rounded-xl p-4 text-center bg-black/30 backdrop-blur-sm">
                  {imagePreview ? (
