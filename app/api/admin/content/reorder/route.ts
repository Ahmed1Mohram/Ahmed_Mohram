...
export async function POST(req: NextRequest) {
  ...
  const { id, lecture_id, direction } = await req.json()
  if (!id || !lecture_id || ![1, -1].includes(direction)) return NextResponse.json({ error: 'invalid' }, { status: 400 })
  ...
  const [u1, u2] = await Promise.all([
    supabaseAdmin.from('lecture_content').update({ order_index: b.order_index }).eq('id', a.id),
    supabaseAdmin.from('lecture_content').update({ order_index: a.order_index }).eq('id', b.id)
  ])
